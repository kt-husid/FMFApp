...
namespace BootstrapWebApplication.BLL
{
    public class ChangeEventHandler
    {
        ...
        private ChangeEvent Get(BootstrapContext db, int? id)
        {
            return db.ChangeEvents.Find(id);
            //return db.ChangeEvents.FilterDeleted().Where(x => x.Id == id).FirstOrDefault();
        }

        public ChangeEvent Update(BootstrapContext db, ChangeEvent changeEvent)
        {
            if (changeEvent == null)
            {
        ...
        public ChangeEvent Create(BootstrapContext db, string createdByUserIdCode = null)//, string updatedByUserIdCode = null, string deletedByUserIdCode = null)
        {
            ...
            return Create(db, DateTime.UtcNow, DateTime.UtcNow, null, createdByUserIdCode, updatedByUserIdCode, null);
        }
        ...
        public ChangeEvent Delete(BootstrapContext db, ChangeEvent changeEvent)
        {
            if (changeEvent != null)
            {
                changeEvent.DeletedOn = DateTime.UtcNow;
