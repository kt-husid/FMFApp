...
from dbo.Member as m JOIN
dbo.Person as p on m.PersonId = p.Id JOIN
dbo.SignOn as so on m.LastSignOnId = so.Id LEFT OUTER JOIN
ChangeEvent as ce ON m.ChangeEventId = ce.Id
where ce.DeletedOn IS NULL AND m.Age >= @AgeFrom AND m.Age <= @AgeTo AND (m.GenderCode = @GenderCode) AND (so.[From] >= @SignOnFrom) AND (so.[To] <= @SignOnTo) AND so.HasInsurance = 'TRUE'
group by m.Age
order by m.Age