...
namespace BootstrapWebApplication.Controllers
{
    public class AddressController : Controller<Address>
    {
        ...
        protected override void AddViewBag(Address address)
        {
            ViewBag.CountryId = new SelectList(db.Countries.ToList().Select(m => new SelectListItem
            {
                Text = m.Name,
                Value = m.Id.ToString()
            }), "Value", "Text", address == null ? 1 : address.CountryId);
        }
        ...
        {
            return GetRelatedDataForParent<Member>(id, "Addresses");
        }
        ...
        {
            Member member = null;
            var result = CreateRelatedObjectUsingViewModel<Member, Address>(vmObj.ParentId, vmObj, (parent) =>
            {
                member = parent;
            ...
            if (member != null)
            {
                var address = member.PrimaryAddress;
                if (address != null)
                {
                    ...
                    member.CountryCode = country != null ? country.Code : "";
                    member.CountryName = country != null ? country.Name : "";
                    member.AddressLine = address != null ? address.AddressLine1 + " " + address.AddressLine2 : "";
                    member.PostalCode = postal != null ? new Nullable<int>(postal.Code) : null;
                    member.CityName = postal != null ? postal.CityName : "";
        ...
        {
            Member member = null;
            var result = UpdateRelatedObjectUsingViewModel<Member, Address>(vmObj.ParentId, vmObj, (parent) =>
            {
                member = parent;
        ...
        {
            //return DeleteObject<Address, Member>(vmObj.Id, vmObj.ParentId);
