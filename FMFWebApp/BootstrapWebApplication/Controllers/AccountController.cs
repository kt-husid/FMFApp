...
namespace BootstrapWebApplication.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            return BootstrapWebApplication.BLL.AccountHandler.Instance.Initialize(User.Identity.Name);
        }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user == null)
                {
                    using (var db = new ApplicationDbContext())
                    {
                        ...
                        //var user2 = user = db.Users.Where(x => x.UserIdCode == model.Email).FirstOrDefault();
                        //user = db.Users.Where(x => x.UserIdCode == model.Email && x.PasswordHash == model.Password).FirstOrDefault();
                        user = db.Users.Where(x => x.UserIdCode.ToLower().Equals(model.Email.ToLower())).FirstOrDefault();
        ...
        public ActionResult Register()
        {
            ...
            //if (user != null)
            //{
            //    if (user.UserIdCode.ToLower().Equals("bha"))
            //    {
            //        return View();
            //    }
            //}
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                using (var db = new ApplicationDbContext())
                {
                    bool canRegister = db.Users.Where(x => x.UserIdCode.ToLower().Equals(model.UserIdCode.ToLower())).FirstOrDefault() == null;
                    if (!canRegister)
                    {
                    ...
                    if (appSettings == null)
                    {
                        appSettings = BootstrapWebApplication.Migrations.Configuration.GetDefaultAppSettings();
        ...
        ApplicationUser AddViewBagData()
        {
            ...
            var printServerURL = user.AppSettings.PrintServerUrl;
            if (!string.IsNullOrEmpty(printServerURL))
            {
                Uri uri = null;
                if (Uri.IsWellFormedUriString(printServerURL, UriKind.Absolute))
                {
                    Uri.TryCreate(printServerURL, UriKind.Absolute, out uri);
                }
                if (uri == null)
                {
            ...
            var client = new RestClient(printServerURL);
        ...
        public ActionResult EditAppSettings()
        {
            ...
            {
                user.AppSettings = BootstrapWebApplication.Migrations.Configuration.GetDefaultAppSettings();
            }
